...
5. $O(n^k)$：多項式複雜度，當輸入規模增長，執行時間呈多項式成長。  
...
| `.map()` 裡面套 `.find()` | $O(n^2)$ | 迴圈套迴圈 |

還有一個常見的寫法類似這樣：`arr.filter().map().reduce()`，可能會覺得其複雜度為 $n \times n \times n = O(n^3)$？  
但其實它只是把陣列跑了三遍，一遍過濾、一遍轉換、一遍計算，每遍都是從頭到尾走一次，加起來是 $3n$，在 Big O 裡常數會被忽略，所以整體仍然是 $O(n)$，不是 $O(3n)$。  
...
之後要查找時，同樣把 key 丟進 hash function，直接算出位置跳過去，就不需要從頭掃，因此查找的複雜度在一般情況下是 $O(1)$。  
...
`querySelector` 本質上就是在這顆樹上透過 DFS (Depth-First Search) 去找節點，一路往最深的子節點鑽，找不到再回頭換下一條路，直到找到第一個符合的元素為止。   
所以在最壞的情況下，`querySelector` 就是把整個 DOM tree 都給爬完，複雜度是 $O(n)$。  
因此有時候會建議把搜尋的範圍縮小，比如：
...
但更根本的作法是透過 `getElementById` 來選取 DOM node。因為 id 在整份文件裡是唯一的，瀏覽器在解析 HTML 時通常會順便維護一張 id → 節點的 hash table，記住每個 id 對應的節點位置。查找時直接從 hash table 拿到該節點的參考，不需要從根節點一路遍歷下去——不過這屬於瀏覽器實作層面的優化，規格本身並沒有強制要求。
...
解決這種在資料量大時極耗效能的方式是改成先建一個「索引簿」（也就是 Hash Table），下面的查找複雜度為 $O(n)$：
...
在這個寫法下，Vue 還是得跑完 `items` 的每一圈去判斷 `v-if` 的條件，就算 1000 筆裡面只有 10 筆是 `active` 的也一樣。  
...
舉例來說 $4 \times n^2$ 跟 $2^n / 4$，一個是 $O(n^2)$、一個是 $O(2^n)$，以複雜度來說會預期前者更好，但在 $n = 11$ 之前，後者都是優於前者的。  
...
### 1NF
第一正規是為了讓每一張表都有一個 primary key，且每個欄位只存一個值。  
...
1. 關聯式資料庫（如 SQL）：設計上鼓勵正規化，將資料拆成多張表透過 id 關聯。要拿完整資料得做 join，API 因此常回傳扁平結構，比如回傳 `authorId` 而不是完整的 `author` 物件。
2. 文件式資料庫（如 MongoDB）：允許將關聯資料直接內嵌成巢狀物件存在同一個 document 裡（反正規化）。少了 join 的需求，API 自然可以一次回傳完整結構。
