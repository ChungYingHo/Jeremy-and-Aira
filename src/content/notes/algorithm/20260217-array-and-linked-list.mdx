...
import MemoryLayoutDiagram from '@/components/ArticleRelated/MemoryLayoutDiagram.svelte'
...
| 類型 | 說明 | 語法範例 |
|------|------|----------|
| 一維陣列 | 最基本的陣列形式，元素排成一列 | `int[] arr = {1, 2, 3};` |
| 二維陣列 | 元素排列成表格或矩陣，有「行」與「列」的概念 | `int[][] m = {{1, 2}, {3, 4}};` |
| 多維陣列 | 超過兩個維度的陣列，可以想成堆疊起來的立體結構 | `int[][][] c = {{{1}, {2}}};` |
...
| 資料結構 | 存放方式 | 擴展性 | 記憶體效率 |
|----------|----------|--------|------------|
| Array | 連續記憶體 | 良好 | 佔用少，但易浪費 |
| Linked List | 不連續記憶體 | 較差 | 佔用多 |
