...
**樹 (Tree)** 是一種非線性資料結構，資料以階層方式延伸，與線性結構（陣列、鏈結串列、堆疊、佇列）不同。典型範例：公司組織圖（一個節點可有多個子節點，但只有一個父節點）。
...
插入：從根節點開始，新值比當前節點小往左走、大往右走，碰到 `null` 就把新節點掛在那裡。搜尋：同樣邏輯，碰到等值回傳命中，碰到 `null` 回傳找不到。

以空樹依序 insert `[5, 3, 7, 1, 4, 6, 8]`：

| insert | 走訪路徑（新值 vs 節點值 → 方向） | 結果樹（括號內為新增點） |
| :---: | :--- | :--- |
| 5 | 空樹 | **(5)** |
| 3 | 3 `<` 5 → 左（空）| 5 / **(3)** |
| 7 | 7 `>` 5 → 右（空）| 5 / 3 / **(7)** |
| 1 | 1 `<` 5 → 左到 3；1 `<` 3 → 左（空）| 5 / (3, 7) / **(1)** |
| 4 | 4 `<` 5 → 左到 3；4 `>` 3 → 右（空）| 5 / (3, 7) / (1, **4**) |
| 6 | 6 `>` 5 → 右到 7；6 `<` 7 → 左（空）| 5 / (3, 7) / (1, 4, **6**) |
| 8 | 8 `>` 5 → 右到 7；8 `>` 7 → 右（空）| 5 / (3, 7) / (1, 4, 6, **8**) |
...
搜尋 `4`：從 5 → 4 `<` 5 左 → 3 → 4 `>` 3 右 → 4 命中。
搜尋 `9`：從 5 → 9 `>` 5 右 → 7 → 9 `>` 7 右 → `null`，回傳不存在。
...
又稱**層序走訪 (Level-order Traversal)**——先把同一層全部拜訪完，再進到下一層。使用佇列 (Queue) 實作。
...
1. **葉節點**：直接刪
2. **只有一個子**：把唯一的子提上來取代
3. **兩個子**：找**中序後繼**（右子樹的最小值）或**中序前驅**（左子樹的最大值），把值複製到被刪節點，再遞迴刪除那個後繼/前驅

以 `5 / 3, 7 / 1, 4, 6, 8` 為例刪除 `5`：右子樹最小 = 6（中序後繼），複製 6 到根、再刪原 6 葉節點 → 結果 `6 / 3, 7 / 1, 4, -, 8`。
