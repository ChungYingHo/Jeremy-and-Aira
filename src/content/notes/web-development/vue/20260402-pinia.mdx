...
原因：`counter.count` 本質上是 `ref`，直接解構取得的是當下的原始值而非 ref 物件。`storeToRefs` 會保留 ref 連結。

---

## Getters — 計算屬性

Getter 等同 `computed`，根據 state 計算衍生資料，具快取特性。
...
Action 是修改 state 的函式。與 Vuex 不同，**Pinia 沒有 mutation**，直接以函式修改 state。
