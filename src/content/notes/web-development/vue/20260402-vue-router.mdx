...
- 但 SPA 還是得處理 URL 對應、瀏覽器上一頁/下一頁、直接輸入網址這些行為——這就是**前端路由**的工作
...
有個坑：從 `/user/123` 導航到 `/user/456`，Vue 會**複用**同一個元件實例，所以 `onMounted` 不會重跑。想跟著參數變化做事，要用 `watch`：
...
導航守衛會在路由切換前攔下來，檢查過了才放行。最常見的用途就是**權限控制**。
