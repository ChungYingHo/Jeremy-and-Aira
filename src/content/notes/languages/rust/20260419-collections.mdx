...
`.iter()` 產生 `&i32`、`.filter()` 內拿到的是 `&&i32`（參考的參考）；`.copied()` 把 `&i32` 複製成 `i32`（`i32` 是 `Copy`，免費）；若是 `String` 則用 `.cloned()`。
...
`Vec` 容量滿時 `push` 會在 heap 上配置更大空間、把舊資料整段搬過去、釋放舊空間。所以指向 Vec 內元素的指標在 `push` 後可能失效——這也是 Rust 借用規則禁止同時持有 `&vec` 與呼叫 `vec.push()` 的理由。
...
字串字面值 `"hello"` 型別是 `&'static str`，指向編譯後 binary 內的唯讀區；`String` 擁有 heap 資料，可增長修改；`&str` 是字串切片，可指向 `String` 的一部分，生命週期必須 $\leq$ 來源。
