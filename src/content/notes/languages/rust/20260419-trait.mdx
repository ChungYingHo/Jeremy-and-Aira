...
Rust 系列最後一篇。前面把資料（型別、struct、enum、集合）跟組織（模組、可見性）都鋪好了，這一篇加最後一塊——「行為」抽象。Trait 是 Rust 的多型機制，兼任 interface、type class 與泛型約束三種角色，連同 `Display`/`Debug` 這類常用 trait 都會在這篇收尾。
...
為什麼 `&dyn Trait` 或 `Box<dyn Trait>`？因為 trait object 的具體型別到執行期才確定（可能是 5 byte 的某 struct、也可能是 200 byte 的另一個），編譯期無法得知 stack 上要預留多大空間。包裝成指標（指標大小固定）後，編譯期就知道大小了。
...
動態派發透過 vtable（函式指標表）呼叫方法；vtable 每個欄位都是單一函式指標，但泛型方法會依 `T` 實體化成多個版本，一張 vtable 放不下；回傳 `Self` 則因不同實作的 `Self` 大小不同，caller 無法預知 stack 佈局。
...
associated type 是 trait 的「內建型別槽」，每個 `impl` 只能填一個值（例如 `Iterator for MyIter` 只能有一種 `Item`）；相比之下泛型 `Iterator<T>` 允許同一個型別對不同 `T` 各實作一次，更彈性但 API 也更囉嗦。Rust 選 associated type 因為大多數情況只需要一種對應關係。
