...
上一篇結尾留了一個伏筆：`String` 與 `&str` 為什麼有「擁有」與「借用」之分。這一篇就是 Rust 最招牌、最讓初學者卡住，但也最值得花時間搞懂的一塊——**所有權系統**。把這套規則裝進腦子，後面寫 struct、Vec、字串處理、錯誤處理才會順。
...
`String` 在 stack 上的三個欄位——`ptr`（指向 heap 上的字元序列起址）、`len`（目前使用的長度）、`cap`（已配置的容量）。move 只複製這三個欄位，heap 上的資料不動。
...
`&mut String` 要先解參考 `*s` 才能整個覆寫內容；或用 `s.make_ascii_uppercase()` 就地改、不需重新配置。
...
掌握這張表 + 三條核心規則，所有權就走過大半了。下一篇進 `struct` 與 `enum`，那邊會看到所有權怎麼跟自訂型別互動，以及 `enum` 配 `match` 的威力。
