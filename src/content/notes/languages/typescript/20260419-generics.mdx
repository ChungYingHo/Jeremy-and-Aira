...
上一篇結尾留下 `identity<T>` 的伏筆。這篇把泛型完整展開——從最基礎的型別參數，一路到 `keyof`、`typeof`、conditional type、`infer`、mapped type、template literal type。下一篇 utility types 與本篇是直接相依，先把這裡的工具吃下去再繼續。
...
泛型 constraint 的 `extends` **不是 class 繼承**，而是「結構相容」檢查：T 必須至少具備 `length: number` 這個欄位。傳 `string`、`number[]`、`{ length: number }` 都 OK，因為它們都有 `length: number`。
...
索引型別 `Arr[number]` 讀的是「陣列型別在任一數字索引位置的元素型別」。例如 `const status = ['ok', 'fail'] as const`，`typeof status` 是 `readonly ['ok', 'fail']`，`typeof status[number]` 就是 `'ok' | 'fail'`。
...
為何 `ToArray<string | number>` 變成 `string[] | number[]`？因為 conditional type `T extends U ? X : Y` 中，若 `T` 是**裸**型別參數（沒包在 tuple 或其他結構裡）且傳入 union，TS 會逐個成員套用條件再 union 起來，稱為 distributive。想關掉這行為，把 `T` 包起來：`[T] extends [U] ? X : Y`。
...
下一篇看標準庫已經把這些泛型工具包成的 utility types（`Partial`、`Pick`、`Omit`、`Record`...），多數時候不必自己寫 mapped type，直接組合就有。
