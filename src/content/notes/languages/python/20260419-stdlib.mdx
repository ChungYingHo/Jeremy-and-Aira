...
Python 賣點之一是「battery included」——標準庫覆蓋極廣，多數場景連 `pip install` 都不用。這篇挑日常每個 Python 程式幾乎都會用到的：`os` / `sys` / `json` / `re` / `datetime` / `collections` / `itertools` / `functools` / `subprocess` / `logging` / `argparse`，整理成一份能當速查的筆記。
...
注意：`groupby` **只把相鄰的相同 key 歸為一組**，不會做整體去重或全域 group。所以幾乎一定要先 `sorted(data, key=...)` 再 `groupby(sorted_data, key=...)`，兩者 key 要一致。否則結果會被打散成多個小組。
