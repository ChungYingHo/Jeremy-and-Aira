...
執行流程：`with` 進入時執行到 `yield`，`yield` 的值綁給 `as` 變數；`with` 區塊結束（正常或 exception）後，程式從 `yield` 繼續執行到 `try/finally` 結束。若區塊內拋 exception，會在 `yield` 處重新 raise——想壓下例外就用 `try/except` 包住 `yield`。
