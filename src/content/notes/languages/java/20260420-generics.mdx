...
前一篇集合框架到處看到 `<T>` / `<K, V>`，這篇展開講泛型本身。Java 泛型有個別語言沒有的歷史包袱——「型別擦除」，所以才會出現 `?`、`extends` / `super` 這套設計。理解擦除就能理解大半的泛型「怎麼這樣寫」。
...
JVM **看不到**泛型，runtime 全被抹成原始型別（`T` → `Object`、`T extends Number` → `Number`）。

「擦除」發生在**編譯期末段**：.java 編譯到 .class 時，`List<String>` 會被替換為 raw `List`，所有 `T` 變成 `Object`（或 bound 上界）。runtime 看到的是沒有泛型資訊的 class 檔——這就是為什麼 `list instanceof List<String>` 不能寫（語法禁止）。
...
「invariant（不變）」在泛型語境指：即使 `Dog extends Animal`，`List<Dog>` 也**不是** `List<Animal>` 的 subtype。反例想像：如果 `List<Dog>` 能傳給吃 `List<Animal>` 的 method，那個 method 可以 `add(new Cat())` 進去——型別系統破功。陣列則是 covariant（協變）、但代價是 runtime 要 `ArrayStoreException` 補檢查。
