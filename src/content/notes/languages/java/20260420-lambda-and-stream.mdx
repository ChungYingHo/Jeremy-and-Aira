...
lazy 的意思：`map`、`filter` 這類**中間操作**只是把動作登記下來、不真的跑；直到**終端操作**（`collect`、`forEach`、`count` 等）被呼叫才一次串起來執行。終端操作完 stream 就「耗盡」了，再呼叫會丟 `IllegalStateException`——stream 不能重用，要重跑就從 source 再開一條。
...
parallelStream 適合「純 CPU 運算、無共享可變狀態、資料量 $> 10^4$ 量級」才划算。I/O 密集（DB、網路）會阻塞 ForkJoinPool 的 worker；小資料或 `forEach` 帶 side effect 都會被同步/fork 的開銷吃掉好處，反而更慢。
...
為什麼欄位/參數/集合元素不要用 Optional？（1）`Optional` 本身可能是 `null`（`Optional<T> x = null`），反而多一層 `NullPointerException` 風險；（2）序列化框架（Jackson、JPA）處理 `Optional` 欄位一向麻煩；（3）`List<Optional<T>>` 比 `List<T>` 又過濾 `null` 語意更清楚。Optional 的定位是回傳型別標示「可能沒有」。
