...
**圖 (Graph)** 是一種非線性資料結構，由**頂點 (Vertex，$V$)** 與**邊 (Edge，$E$)** 組成，描述物件之間的關係，是資料結構中表達能力最強的一種。後面常用 $|V|$ 代表頂點數、$|E|$ 代表邊數。
