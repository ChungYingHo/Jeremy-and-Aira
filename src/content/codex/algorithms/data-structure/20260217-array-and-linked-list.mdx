...
import MemoryLayoutDiagram from '@/components/ArticleRelated/data-structure/array/MemoryLayoutDiagram.svelte'
import Array2DDiagram from '@/components/ArticleRelated/data-structure/array/Array2DDiagram.svelte'
...
<MemoryLayoutDiagram client:only="svelte" />
...
| 類型 | 宣告語法 | 說明 |
|------|----------|------|
| 一維陣列 (1D) | `int[]` | 最基本的陣列，元素排成一列，如一排學生座號。 |
| 二維陣列 (2D) | `int[][]` | 元素排成表格，有行與列的概念，如教室座位表。 |
| 多維陣列 (nD) | `int[][][]` | 三個以上維度，可想像成多間教室堆疊的立體結構。 |

二維陣列的本質是「陣列的陣列」，用 `matrix[列][行]` 的方式存取每一格：

<Array2DDiagram client:only="svelte" />
...
| 資料結構 | 讀取 | 搜尋 | 插入 | 刪除 | 存放方式 |
|----------|:----:|:----:|:----:|:----:|----------|
| Array | $O(1)$ | $O(n)$ | $O(n)$ | $O(n)$ | 連續記憶體 |
| Linked List | $O(n)$ | $O(n)$ | $O(1)$ | $O(1)$ | 不連續記憶體 |
