...
<div class="w-full max-w-2xl mx-auto my-8 bg-cream rounded-xl border border-line shadow-sm overflow-hidden">
  <svg viewBox="0 0 {W} {H}" class="w-full" font-family="sans-serif">
    <text x={W / 2} y="18" fill="#5d574d" font-size="10" text-anchor="middle">
      二維平面上的極點（Maximal Points）
    </text>

    <!-- grid -->
    {#each Array(8) as _, i}
      <line x1={toX(i + 1)} y1={toY(1)} x2={toX(i + 1)} y2={toY(8)} stroke="#e3dfd5" stroke-width="0.5"/>
      <line x1={toX(1)} y1={toY(i + 1)} x2={toX(8)} y2={toY(i + 1)} stroke="#e3dfd5" stroke-width="0.5"/>
    {/each}

    <!-- axes -->
    <line x1={toX(1) - 10} y1={toY(1)} x2={toX(8) + 10} y2={toY(1)} stroke="#b0a99a" stroke-width="1"/>
    <line x1={toX(1)} y1={toY(1) + 10} x2={toX(1)} y2={toY(8) - 10} stroke="#b0a99a" stroke-width="1"/>
    <text x={toX(8) + 18} y={toY(1) + 3} fill="#8a8475" font-size="9">x</text>
    <text x={toX(1) - 4} y={toY(8) - 14} fill="#8a8475" font-size="9">y</text>

    <!-- axis ticks -->
    {#each [1, 2, 3, 4, 5, 6, 7, 8] as v}
      <text x={toX(v)} y={toY(1) + 14} fill="#8a8475" font-size="8" text-anchor="middle">{v}</text>
      <text x={toX(1) - 10} y={toY(v) + 3} fill="#8a8475" font-size="8" text-anchor="middle">{v}</text>
    {/each}
    ...
    {#if step.divider != null}
      ...
      <text x={toX(step.divider) + 6} y={toY(8) - 4} fill="#3f5f7a" font-size="8">R</text>
    {/if}

    <!-- dominance arrows -->
    <defs>
      <marker id="dom-arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="5" markerHeight="5" orient="auto">
        <path d="M 0 0 L 10 5 L 0 10 z" fill="#c46a5c"/>
      </marker>
    </defs>
    {#each step.arrows as arr}
      ...
      <line x1={toX(p.x)} y1={toY(p.y)} x2={toX(q.x)} y2={toY(q.y)}
        stroke="#c46a5c" stroke-width="1.2" stroke-dasharray="2 2" opacity="0.9"
    ...
    {#if step.staircase}
      ...
      <polyline
        ...
        fill="none" stroke="#5f7355" stroke-width="1.5" stroke-dasharray="4 2" opacity="0.85"/>
    {/if}
    ...
    {#each points as pt, i}
      ...
      {#if isMax}
        <rect x={toX(pt.x) - 7} y={toY(pt.y) - 7} width="14" height="14"
          fill="rgba(95, 115, 85, 0.18)" stroke="#5f7355" stroke-width="1.5"/>
      {/if}
      <circle cx={toX(pt.x)} cy={toY(pt.y)} r={isHi ? 5.5 : 4.5}
        fill={isMax ? '#e6ece2' : isDom ? '#f3ddd8' : '#f0eee8'}
        stroke={isMax ? '#5f7355' : isDom ? '#c46a5c' : isHi ? '#6e94b5' : '#d6d1c5'}
        stroke-width="1.5"/>
      <text x={toX(pt.x) + 9} y={toY(pt.y) - 7}
        fill={isMax ? '#3f5239' : isDom ? '#9c4a3d' : '#5d574d'}
        font-size="9">{pt.id}</text>
    {/each}

    <!-- legend -->
    <rect x="16" y="307" width="10" height="10" fill="rgba(95, 115, 85, 0.18)" stroke="#5f7355" stroke-width="1.2"/>
    <text x="30" y="316" fill="#5d574d" font-size="8.5">極點</text>
    <circle cx="72" cy="312" r="4" fill="#f3ddd8" stroke="#c46a5c" stroke-width="1.2"/>
    <text x="80" y="316" fill="#5d574d" font-size="8.5">被宰制</text>
    <line x1="124" y1="312" x2="142" y2="312" stroke="#c46a5c" stroke-width="1.2" stroke-dasharray="2 2"/>
    <text x="146" y="316" fill="#5d574d" font-size="8.5">宰制關係</text>

    <!-- step desc -->
    <text x={W / 2} y="334" fill="#3f3a31" font-size="10" text-anchor="middle">{step.desc}</text>
  </svg>
