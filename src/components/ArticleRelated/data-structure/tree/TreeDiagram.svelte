<script>
  ...
  $: {
    ...
    nodeDisplay = nodes.map(n => ({
      ...n,
      fill:   n.id === cur      ? '#f3e3dc'
            : visited.has(n.id) ? '#e2eaf0'
            :                     '#f0eee8',
      stroke: n.id === cur      ? '#b3674a'
            : visited.has(n.id) ? '#6e94b5'
            :                     '#d6d1c5',
      text:   n.id === cur      ? '#8a4a32'
            : visited.has(n.id) ? '#3f5f7a'
            :                     '#5d574d',
    }))
    ...
    edgeDisplay = rawEdges.map(([a, b]) => ({
      ...
      color: visited.has(a) && visited.has(b) ? '#6e94b5' : '#d6d1c5',
    }))
  }
